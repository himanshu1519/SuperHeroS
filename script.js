...
const loadFavlist = () => {
    ...
    favArray.forEach(async function (item) {
        const result = await fetch(
            `https://www.superheroapi.com/api.php/107062348801325/${item}`
        );

        // https://www.superheroapi.com/api.php/107062348801325/search/${searchText}
        const heroDetails = await result.json();
        // console.log(heroDetails)
        const divElem = document.createElement("div");
        divElem.classList.add("search-list-item");
        divElem.innerHTML = `
            <img src = "${heroDetails.image.url ? heroDetails.image.url : ""
            }" alt = "">
...
// api key = 107062348801325 (taken the api key from https://superheroapi.com/ with login facebook)
const fetchAllSuperHero = async (searchText) => {
    let url = `https://www.superheroapi.com/api.php/107062348801325/search/${searchText}`;
    try {
...
//fetching the name and image's of SuperHero's
...
// getting the id's of the superheros
...
favBtn.addEventListener("click", (e) => {
    ...
    divElem.innerHTML = `
              <img src = "${singleData[0].image.url ? singleData[0].image.url : ""
        }" alt = "">
